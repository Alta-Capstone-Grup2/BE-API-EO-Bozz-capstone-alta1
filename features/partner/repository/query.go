...
func (repo *partnerRepository) GetOrders(partnerID uint) (data []partner.OrderCore, err error) {
	...
	// tx := repo.db.Where("service_name LIKE ?", "%"+queryServiceName+"%").Where(&Service{City: queryCity, ServiceCategory: queryPServiceCategory, ServicePrice: queryServicePrice}).Find(&modelData)
	tx := repo.db.Raw("SELECT `orders`.`id`,`orders`.`event_name`,`orders`.`start_date`,`orders`.`end_date`,`orders`.`event_location`,`orders`.`event_address`,`orders`.`note_for_partner`,`orders`.`service_name`,`orders`.`service_price`,`orders`.`gross_ammount`,`orders`.`payment_method`,`orders`.`order_status`,`orders`.`payout_reciept_file`,`orders`.`payout_date`,`orders`.`service_id`,`orders`.`client_id` FROM `services` JOIN `partners` ON `services`.`partner_id` = `partners`.`id` JOIN `orders` ON `orders`.`service_id` = `services`.`id` ORDER BY CASE `orders`.`order_status` WHEN 'waiting confirmation' THEN 1 WHEN 'order confirmed' THEN 2 WHEN 'complete order' THEN 3 WHEN 'paid off' THEN 4 ELSE 5 END ASC,`orders`.`start_date` DESC;").Where("`partners.id` = ?", partnerID).Scan(&modelData)
